...
io.on('connection', (socket) => {
    ...
    // Validation helper to prevent array broadcast abuse and DoS via large inputs
    const isValidId = (id) => typeof id === 'string' && id.length > 0 && id.length <= 100;

    // When a user joins a room
    socket.on('join-room', (roomId) => {
        if (!isValidId(roomId)) return; // Security fix: validate roomId
        socket.join(roomId);
    ...
    socket.on('offer', (offer, roomId, toId) => {
        if (!isValidId(toId)) return; // Security fix: validate toId
